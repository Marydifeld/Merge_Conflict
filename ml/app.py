...
# Conectar a MongoDB (opcional — si falla, se usan valores por defecto)
col_clientes = None
try:
    cliente_mongo = MongoClient(MONGO_URI, serverSelectionTimeoutMS=3000)
    db            = cliente_mongo[MONGO_DB]
    col_clientes  = db[MONGO_COLLECTION]
    col_clientes.find_one()  # verificar conexión
    print("MongoDB conectado OK")
except Exception as e:
    print(f"MongoDB no disponible, se usarán valores por defecto: {e}")
